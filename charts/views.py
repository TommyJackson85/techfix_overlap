...
"""
graphs showing how many bugs or features are tended to on a daily, weekly and monthly basis
"""
...
"""
Most the code logic is my own.
"""

def get_most_voted(objects):
    top_five_voted = {'votes': []}
...
def get_averages(objects, now, all_days, user):
    dates = []
    """collects dates over specific time periods,
    in this case, posts progress dates between start time and end time. 
    If post doesnt have an end time it takes present day"""
    for p in objects:
    ...
    amount_per_week = []
    amount_per_month = []
    """sees which dates overlaps with all days( last 3 months ), then adds them to amount per day array"""
    for a in all_days:
        num = 0
        for b in dates:
            if a in b:
    ...
    weekly_summary = []
    """calculates weekly summary using a simular process to getting daily summaries"""
    for w in all_weeks:
        if len(w) == 7:
            num = 0
            for d in dates:
    ...
    """calculates monthly summary using a simular process to getting daily summaries"""   
    monthly_summary = []
    for m in all_months:
        if len(m) == 30:
            num = 0
...
def get_charts(request):
    ...
    if bug_posts_votes:
        """calls get most voted function from above"""
